...
function getPositionType() {
    return {
        ...
        rgo: "Руководитель группы офисов",
    }
}
...
function getPersonMessage(message) {
    ...
    return (
        ...
        "мы назначили тебе индивидуальный трек обучения.</div><br>" +
        "<div>Он поможет достичь лучших результатов " +
        "в продажах этих продуктов.</div>" +
        "<div>Пройди обучение до " +
        StrDate(card.plan_readiness_date, false, false) + " по " +
        "<a href=\"" + link + "\">ссылке<a>.</div>" +
...
function getDmMessage(message) {
    ...
    return (
        ...
        "</div>" +
        "<br>" +
        "<div>Проконтролировать прохождение можно по " +
        "<a href=\"" + link + "\">ссылке<a>.</div>" +
        "<br>" +
...
try {
    ...
    var params = createParams(Date("2026-05-01"))
